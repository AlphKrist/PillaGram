...
const Taletimeb4quiz = () => {
  const navigation = useNavigation();
  const { user } = useGlobalContext();
  const courseId = 'taletimeCourse';
  const lessonId = 'taletimeb4quiz';
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Text */}
      <Text style={styles.title}>The Mysterious Forest{"\n"}(Using Context Clues)</Text>
      
      {/* Start Button */}
      <TouchableOpacity 
        style={styles.startButton}
        onPress={() => router.push("/taletimeCourse/taletimeQuiz")} // Navigate to the quiz screen
      >
